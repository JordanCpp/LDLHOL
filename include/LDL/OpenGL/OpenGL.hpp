...
void LDL_GL_Check(const char* file, int line, const char* expression)
{
	...
	if (code != GL_NO_ERROR)
	{
		const size_t error_max = 32;
		char error[error_max];
		LDL_BytesClear(error, 0, sizeof(error));

		switch (code)
		{
		case GL_INVALID_ENUM:
			LDL_CharsCopy(error, "GL_INVALID_ENUM", error_max);
			break;
		case GL_INVALID_VALUE:
			LDL_CharsCopy(error, "GL_INVALID_VALUE", error_max);
			break;
		case GL_INVALID_OPERATION:
			LDL_CharsCopy(error, "GL_INVALID_OPERATION", error_max);
			break;
		case GL_STACK_OVERFLOW:
			LDL_CharsCopy(error, "GL_STACK_OVERFLOW", error_max);
			break;
		case GL_STACK_UNDERFLOW:
			LDL_CharsCopy(error, "GL_STACK_UNDERFLOW", error_max);
			break;
		case GL_OUT_OF_MEMORY:
			LDL_CharsCopy(error, "GL_OUT_OF_MEMORY", error_max);
			break;
		default:
			LDL_CharsCopy(error, "Unknown error", error_max);
		}

		LDL_Console console;
		LDL_Converter conv;

		const size_t buffer_max = 128;
		char buffer[buffer_max];
		LDL_BytesClear(&buffer, 0, buffer_max);

		LDL_CharsConcat(buffer, "OpenGL Error: ", buffer_max);
		LDL_CharsConcat(buffer, error, buffer_max);
		LDL_CharsConcat(buffer, " File: ", buffer_max);
		LDL_CharsConcat(buffer, file, buffer_max);
		LDL_CharsConcat(buffer, " Line: ", buffer_max);
		LDL_CharsConcat(buffer, conv.Convert(line), buffer_max);
		LDL_CharsConcat(buffer, " Detail: ", buffer_max);
		LDL_CharsConcat(buffer, expression, buffer_max);
