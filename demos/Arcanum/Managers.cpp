...
PathManager::PathManager(const char* path)
{
	...
	LDL_CharsCopy(_ShortPath, path);
}

const char* PathManager::Path(const char* dir, const char* file)
{
	size_t length = LDL_CharsLength(_ShortPath) + LDL_CharsLength(dir) + LDL_CharsLength(file);

	LDL_ASSERT(length < PathManager::FullMax);

	LDL_BytesClear(_FullPath, 0, sizeof(_FullPath));

	LDL_CharsCopy(_FullPath, _ShortPath);
